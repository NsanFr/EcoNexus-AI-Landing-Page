...
function AudienceCard({
  ...
  return (
    <div
      ref={ref}
      className="border border-border/50 rounded-md p-5 sm:p-6 shadow-sm bg-card"
      style={{
        opacity: inView ? 1 : 0,
    ...
    >
      <h3 className="font-sans text-sm sm:text-base font-semibold text-text mb-3 sm:mb-4">
        {audience.title}
      </h3>
      <ul className="space-y-2 sm:space-y-3">
        {audience.items.map((item) => (
          <li
            key={item}
            className="text-xs sm:text-sm text-text/70 flex items-start gap-2"
          >
...
export default function TargetAudience() {
  ...
  return (
    <section id="dampak" className="px-6 py-20 sm:py-24 md:py-32 bg-card scroll-mt-20">
